/**
 * Template HTML pour bon de commande PVC PORTE RÉNOVATION
  ...
  return `
    <div class="menuiserie-bloc">
      ...
      <div class="section">
        <h3>TYPE DE PORTE</h3>
        <div class="field-row">
          <span>${checkbox(data.typeProduit === 'PE 1 Vtl.')} PE 1 Vtl.</span>
          <span>${checkbox(data.typeProduit === 'PE 2 Vtx.')} PE 2 Vtx.</span>
          <span>${checkbox(data.typeProduit === 'PS 1 Vtl.')} PS 1 Vtl.</span>
          <span>${checkbox(data.typeProduit === 'PS 2 Vtx.')} PS 2 Vtx.</span>
        </div>
        <div class="field-row">
          <span>${checkbox(data.ouverture === 'Extérieure')} Ouverture Extérieure</span>
        </div>
      </div>
      ...
      <div class="section">
        <h3>DORMANT</h3>
        <div class="field-group">
          <span class="label">Type d'aile:</span>
          <span class="value">${data.dormant || ''}</span>
        </div>
        ${data.repriseDoublage ? `
        <div class="field-row">
      ...
      <div class="section">
        ...
        <div class="field-row">
          ...
          <span>${checkbox(data.fixation === 'Vis')} Vis</span>
          <span>${checkbox(data.fixation === 'Pattes Std')} Pattes Std</span>
          <span>${checkbox(data.fixation === 'Pattes PRDT')} Pattes PRDT</span>
        </div>
        ${data.seuil ? `
        <div class="field-row">
          <span class="label">Seuil:</span>
          <span class="value">${data.seuil}</span>
        </div>
        ` : ''}
        ${data.compensateur ? `
        <div class="field-row">
          <span class="label">Compensateur:</span>
          <span class="value">${data.compensateur}</span>
        </div>
        ` : ''}
      </div>
      ...
      <div class="section">
        <h3>REMPLISSAGE</h3>
        <div class="field-row">
          <span>${checkbox(data.soubassement === 'Vitré')} Vitré</span>
          <span>${checkbox(data.soubassement === 'Plein lisse')} Plein lisse</span>
          <span>${checkbox(data.soubassement === 'Plein rainuré')} Plein rainuré</span>
        </div>
        ${data.vitrageDecoratif ? `
        <div class="field-row">
          <span class="label">Vitrage déco:</span>
          <span class="value">${data.vitrageDecoratif}</span>
        </div>
        ` : ''}
      </div>
      ...
      <div class="section">
        <h3>VITRAGE</h3>
        <div class="field-row">
          <span>Épaisseur: ${checkbox(data.epaisseurVitrage === '26 mm')} 26mm ${checkbox(data.epaisseurVitrage === '18 mm')} 18mm ${checkbox(data.epaisseurVitrage === '45 mm')} 45mm</span>
        </div>
        <div class="field-row">
          <span class="label">Standard:</span>
          <span class="value">${data.vitrageStandard || 'Std : 4/20/4 PTR+'}</span>
        </div>
        <div class="field-row">
      ...
      <div class="section">
        <h3>QUINCAILLERIE</h3>
        <div class="field-row">
          <span>${checkbox(data.cylindre === 'F6S')} F6S Sécurité</span>
          <span>${checkbox(data.cylindre === 'RX16')} RX16 Haute Sécurité</span>
        </div>
        <div class="field-group">
          <span class="label">Poignée:</span>
          <span class="value">${data.poignee || ''}</span>
        </div>
        ${data.barresDeTirage ? `
        <div class="field-row">
          <span class="label">Barre de tirage:</span>
          <span class="value">${data.barresDeTirage}</span>
        </div>
        ` : ''}
      </div>
      ...
      <div class="section">
        ...
        <div class="field-row">
          ...
          <span class="value">${data.habillageInt || 'Standard'}</span>
        </div>
        <div class="field-row">
          <span class="label">Ext:</span>
          <span class="value">${data.habillageExt || 'Standard'}</span>
        </div>
      </div>
...
export function renderBonCommandePvcRenoPorte(
    ...
    <div class="header">
      ...
      <div class="color-section">
        ...
        <div class="field-row">
          ...
          <span class="value">${firstData.finition || ''}</span>
        </div>
        <div class="field-row">
          <span class="label">Mode:</span>
          <span>${checkbox(firstData.modeColoris === 'Monocolore')} Monocolore</span>
          <span>${checkbox(firstData.modeColoris === 'Bicolore')} Bicolore</span>
        </div>
        ${firstData.modeColoris === 'Monocolore' ? `
        <div class="field-row">
          <span class="label">Teinte:</span>
          <span class="value">${firstData.couleurInt || firstData.couleurExt || ''}</span>
        </div>
        ` : ''}
        ${firstData.modeColoris === 'Bicolore' ? `
        <div class="field-row">
          <span class="label">Int:</span>
